...
async function hasCorrectSendAmount(
  receipt: string,
  amount: string,
  currency: string
): Promise<boolean> {
...
async function hasNameInReceipt(
  receipt: string,
  userName: string
): Promise<boolean> {
...
async function parseReceiptId(
  ...
  const base64Raw = receipt.slice(
    referenceIndex + REFERENCE_LINK_PATTERN.length + 3,
    referenceIndex + REFERENCE_LINK_PATTERN.length + 100
  );
  const b64Link = emlformat.unquotePrintable(base64Raw);

  const receiptId = Buffer.from(b64Link, "base64")
    .toString("utf-8")
    .split("/")
    .slice(-1)[0]
    .match(/^[0-9]+/g);
...
const worker = new Worker(
  ...
  async (job) => {
    try {
      ...
      //   const hasCorrectAmountUSD = await hasCorrectSendAmount(
      //     receipt,
      //     amountToReceive,
      //     currency_usd
      //   );
      //   const currency_cad = "CAD";
      //   const hasCorrectAmountCAD = await hasCorrectSendAmount(
      //     receipt,
      //     amountToReceive,
      //     currency_cad
      //   );
      ...
      //   console.log(
      //     `Attempting to prove receipt for order ${orderId} with value ${amountToReceive} from ${sellerName} `
      //   );
